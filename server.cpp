...
void clkaes_encryption() {
    ...
    // Generate elements
    element_t C1, C2, K1, K2, P, xIDS, PIDS;
    element_init_G1(C1, pairing);
    element_init_G1(C2, pairing);
    element_init_GT(K1, pairing);
    element_init_G1(K2, pairing);
    element_init_G1(P, pairing);
    element_init_Zr(xIDS, pairing);
    element_init_G1(PIDS, pairing);

    // Assume IDS, PIDS, and xIDS are initialized
    element_t IDS;
    // Initialize IDS and PIDS elements

    // Compute P = xIDS * PIDS
    element_mul(P, xIDS, PIDS);
    ...
    // Compute K1 = e(DIDS, H(IDR)) where DIDS is the partial private key of IDS
    element_t DIDS, H_IDR;
    element_init_G1(DIDS, pairing);
    element_init_G1(H_IDR, pairing);
    // Compute DIDS and H(IDR) based on IDS and IDR

    element_pairing(K1, DIDS, H_IDR);

    // Compute K2 = xIDS * PIDR where PIDR is the public key of IDR
    element_t PIDR;
    element_init_G1(PIDR, pairing);
    // Initialize PIDR based on IDR

    element_mul(K2, xIDS, PIDR);

    // Compute h2(h1(IDS, PIDS, IDR, PIDR, K1, K2, w), C1)P
    element_t h1_result, h2_result;
    ...
    // Compute h2(h1_result, C1) and then multiply by P to get C2
